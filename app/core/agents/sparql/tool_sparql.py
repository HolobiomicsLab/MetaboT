...
class GraphSparqlQAChain(KgbotBaseTool):
    ...
    graph: RdfGraph = None
    # added parameter specifying if additional parameters are required
    ...
    def _run(
        ...
        #############NEW CODE########################################
        # Check if the result is empty+add staff to generate the query again
        if not result:
            ...
            logger.info("Regenerated SPARQL query: %s", generated_sparql)
